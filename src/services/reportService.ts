...
class ReportService {
  ...
  async createReport(data: CreateReportDTO) {
    try {
      ...
      const generatedData = await reportFactoryService.generateReportData(data.studentsData);
      ...
      const normalizedStudents = data.studentsData.map((student, index) => ({
        ...
        reading: Number(student.READING),
        listening: Number(student.LISTENING),
        speaking: Number(student.SPEAKING),
        writing: Number(student.WRITING),
        speakingFeedback: student['FEEDBACK SPEAKING']?.toString() || '',
        writingFeedback: student['FEEDBACK WRITING']?.toString() || '',
        recommendations: JSON.stringify(generatedData.recommendations.individual[index])
      }));
      ...
      const response = await fetch(`${this.API_BASE_URL}/reports`, {
        ...
        body: JSON.stringify({
          ...
          recommendations: JSON.stringify(generatedData.recommendations.general),
          distribution: JSON.stringify(generatedData.distribution),
  ...
  async getStudentReportByToken(token: string) {
    try {
      const response = await fetch(`${this.API_BASE_URL}/shared/student/${token}`);
      if (response.status === 404) return null;
      return await this.handleResponse(response);
    } catch (error) {
